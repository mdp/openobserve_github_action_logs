...
import {parseLog} from '../src/action'
...
`
  const job = {id: 123, run_id: 234234}
  const result = parseLog(job, log)
...
it('should return ignore invalid dates', () => {
  const log =
    '2023-07-25T13:BA:09.123456Z [INFO] [elastic] [run.py:main:25] [metadata_url] https://api.github.com/repos/elastic/observability-ci/actions/runs/1000'
  const job = {id: 123, run_id: 234234}
  const result = parseLog(job, log)
...
it(`should read from the fixtures log file,
  ...
  const log = await readFile(join(__dirname, 'fixtures', 'gh_logs.txt'), 'utf8')
  const job = {id: 123, run_id: 234234}
  const result = parseLog(job, log)
  console.log(result)
